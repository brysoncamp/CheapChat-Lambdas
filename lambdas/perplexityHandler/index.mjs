...
// Fetch Perplexity Response with strict ordering
    ...
    const req = https.request(options, (res) => {
      ...
      res.on("data", async (chunk) => {
        ...
        if (boundary !== -1) {
          ...
          for (const message of completeMessages.split("\n")) {
            if (message.trim()) {
              sequence++;
              await processMessage(message, connectionId, sessionId, sequence);
            }
          }
        }
      });
      ...
      res.on("end", async () => {
        if (buffer.trim().length > 0) {
          sequence++;
          await processMessage(buffer, connectionId, sessionId, sequence);
        }
        resolve();
      });
    });
...
// Process each message and enforce strict ordering
